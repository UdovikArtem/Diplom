        ...
        if (parent != null)
        {
            ...
            if (enemy != null)
            {
                ...
                else if ((div > 0 && div > 1) || (div < 0 && div < -1))
                {
                    ...
                    {
                        enemy.setAttackDir(new Vector3(scale.x, scale.y * (1 / div), scale.z));
                    }
                    else
                    {
                        enemy.setAttackDir(new Vector3(scale.x, scale.y * (-1 / div), scale.z));
                ...
                else if ((div > 0 && div < 1) || (div < 0 && div > -1))
                {
                    if ((enemy.Target.position.y - parent.position.y) > 0)
                    {
                        enemy.setAttackDir(new Vector3(scale.x * Math.Abs(div), scale.y, scale.z));
                    }
                    else
                    {
                        enemy.setAttackDir(new Vector3(scale.x * Math.Abs(div), scale.y * -1, scale.z));
                    }
                }
